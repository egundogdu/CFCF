function [net, stats] = CFCF_train(net, imdb, getBatch, NoEpochs)
opts.expDir = fullfile('exp_vgg_finetuned') ;
opts.continue = true ;
opts.batchSize = 128 ;
...
opts.gpus = [1] ;
opts.prefetch = false ;
opts.numEpochs = NoEpochs ;

temptemp = repmat(logspace(-5,-6, 20), [100 1]);
opts.learningRate = temptemp(:)';
% opts.learningRate = logspace(-6,-8,50);
...
randInd = 1:size(imdb.patches,4); %randperm(size(imdb.patches,4));
opts.train = randInd(1:uint16(0.8*length(randInd)));
opts.val = randInd(uint16(0.8*length(randInd))+1:end);
...
modelFigPath2 = fullfile(opts.expDir, 'net-train.fig') ;
...
% -------------------------------------------------------------------------
[rs, cs] = ndgrid((1:101) - 51, (1:101) - 51);
g = single(exp(-0.5 * (((rs.^2 + cs.^2) / 7^2))));
G = fft2(g);
...
for t=1:opts.batchSize:numel(subset)
  ...
  for s=1:opts.numSubBatches
    ...
    [patches, filters, desired] = state.getBatch(state.imdb, batch) ;
    imdb_temp.patches = patches;
    imdb_temp.filters = filters;
    imdb_temp.desired = desired;
    ...
    if numGpus >= 1
%       im = gpuArray(im) ;
      imdb_temp.patches = gpuArray(imdb_temp.patches);
      imdb_temp.filters = gpuArray(imdb_temp.filters);
    end
    ...
    [LossVal, estimated, resX, resY, ErrorSig, delLH, delLX, delLY] = ...
        DCFCF_forwardhalfback( net, imdb_temp, G, 1:size(imdb_temp.filters,4), istrain);
    
    delLX = gpuArray(delLX); delLY = gpuArray(delLY);
    if istrain
        resX1 = vl_simplenn2(1, net, imdb_temp.patches, delLX, resX, ...
                      'accumulate', s ~= 1, ...
                      'mode', evalMode, ...
                      ...
                      'sync', opts.sync, ...
                      'cudnn', opts.cudnn) ;
        resY1 = vl_simplenn2(1, net, imdb_temp.filters, delLY, resY, ...
                      'accumulate', s ~= 1, ...
                      'mode', evalMode, ...
                      'conserveMemory', opts.conserveMemory, ...
                      'backPropDepth', opts.backPropDepth, ...
...
%       reshape(opts.errorFunction(opts, labels, res),[],1) ; ]],2) ;
  ...
  if strcmp(mode, 'train')
    ...
    [state, net] = accumulate_gradients(state, net, resX1, opts, batchSize, mmap) ;
    [state, net] = accumulate_gradients(state, net, resY1, opts, batchSize, mmap) ;
  end
...
% -------------------------------------------------------------------------
load(['YOURLOCAL/' fileName]);%, 'net', 'stats') ;
